...
const WIDE_TOP_DOWN_CAMERA_SIZE=15.6
...
const WIDE_BOARD_SIZE=Vector2(23,18.4)
const TOP_DOWN_BOARD_SIZE=WIDE_BOARD_SIZE
...
const FIELD_EXTENSION=1.2
const FIELD_TOP_CUT=0.375
const FIELD_BOTTOM_CUT=0.625
const WIDE_BACK_ROW_SPLIT=0.9
const WIDE_ZONES={
 "unit":Rect2(-8.85,0.0,17.7,2.15+FIELD_EXTENSION),
 "item":Rect2(-8.85,2.15+FIELD_EXTENSION,8.85+WIDE_BACK_ROW_SPLIT,2.15),
 "support":Rect2(WIDE_BACK_ROW_SPLIT,2.15+FIELD_EXTENSION,8.85-WIDE_BACK_ROW_SPLIT,2.15),
 "palette":Rect2(-8.85,4.42+FIELD_EXTENSION,17.7,3.36),
...
func build(engine, provider: Callable, mat_path: String, payment_reservations: Array=[], selection: Array=[]):
 ...
 env.background_color=Color("#1c1c35")
 ...
 if wide_playmat:
  ...
  backdrop_material.albedo_texture=preload("res://assets/hakurei_sunset/shrine_backdrop.png")
  ...
  plane(self,Vector3(0,-0.22,0),Vector2(73,52),backdrop_material).name="ShrineBackdrop"
  ...
  wide_mat.set_shader_parameter("sunset_background",preload("res://assets/hakurei_sunset/sunset_atmosphere.png"))
  ...
  wide_mat.set_shader_parameter("source_height",PRINTED_MAT_SIZE.y/WIDE_BOARD_SIZE.y)
  wide_mat.set_shader_parameter("insert_height",FIELD_EXTENSION/WIDE_BOARD_SIZE.y)
  wide_mat.set_shader_parameter("top_cut",FIELD_TOP_CUT)
  wide_mat.set_shader_parameter("bottom_cut",FIELD_BOTTOM_CUT)
  mat=wide_mat
 else:
  ...
  mat=plain_mat
 board=plane(self,Vector3.ZERO,displayed_board_size(),mat); board.name="Playmat3"
...
func add_wide_zone_guides():
 var field_guide=guide_material(Color(0.84,0.78,0.91,0.52))
 var palette_guide=guide_material(Color(0.88,0.51,0.52,0.53))
 for who in range(2):
  var side=1.0 if who==local_seat else -1.0
  guide_horizontal(-8.85,8.85,(2.15+FIELD_EXTENSION)*side,field_guide)
  guide_vertical(WIDE_BACK_ROW_SPLIT*side,minf((2.15+FIELD_EXTENSION)*side,(4.3+FIELD_EXTENSION)*side),maxf((2.15+FIELD_EXTENSION)*side,(4.3+FIELD_EXTENSION)*side),field_guide)
  guide_horizontal(-8.85,8.85,(4.42+FIELD_EXTENSION)*side,palette_guide)
  guide_horizontal(-8.85,8.85,(7.78+FIELD_EXTENSION)*side,palette_guide)
  for x in [-8.85,8.85]:
   guide_vertical(x,minf(0.0,(7.78+FIELD_EXTENSION)*side),maxf(0.0,(7.78+FIELD_EXTENSION)*side),field_guide)
...
func displayed_board_size() -> Vector2:
 ...
 return WIDE_BOARD_SIZE if wide_playmat else BOARD_SIZE
...
func zone_position(zone: String, who: int) -> Vector3:
 ...
 if MAT_SLOTS.has(slot):
  ...
  if wide_playmat:
   return Vector3(playmat_x(uv.x)*side,0.035,((uv.y-0.5)*PRINTED_MAT_SIZE.y+FIELD_EXTENSION*signf(uv.y-0.5))*side)
 ...
 match zone:
  ...
  "palette": return Vector3(0,0.035,((TOP_DOWN_PALETTE_Z+FIELD_EXTENSION) if wide_playmat else TOP_DOWN_PALETTE_Z if top_down_view else 6.8)*side)
 return Vector3.ZERO

func card_snapshot() -> Dictionary:
 var result={}
 for p in duel.players:
  for leader in [p.leader]+p.get("extra_leaders",[]):result[leader.uid]=leader.duplicate(true)
  ...
  for group in groups:
   ...
   for c in groups[group]:
    ...
    if not signature.is_empty() and keys.has(signature):bundles[keys[signature]].append(c)
     ...
     bundles.append([c])
   var list=bundles.map(func(b):return b[0])
   var duplicate_totals={};var duplicate_seen={}
   if group=="support":
    for c in list:
     if not duel.cards[c.card_id].get("stackable",false):duplicate_totals[c.card_id]=duplicate_totals.get(c.card_id,0)+1
   for i in range(list.size()):
    var c=list[i]
    var placement=support_placement(i,list.size(),who) if group=="support" else {"at":field_position(group,i,list.size(),who),"scale":FIELD_SCALE}
    var d=description(c,placement.at,placement.scale)
    d.group=group;d.members=bundles[i].map(func(u):return u.uid)
    if group in ["support","melody"]:d.rotation.y+=PI/2
    if group=="support":d.fanned=placement.fanned
    if duplicate_totals.get(c.card_id,0)>1:
     duplicate_seen[c.card_id]=duplicate_seen.get(c.card_id,0)+1
...
func support_bounds() -> Rect2:
 if wide_playmat:return WIDE_ZONES.support
 if top_down_view:return Rect2(1.2,2.05,6.15,2.4)
 return Rect2(0.35,3.12,7.1,2.38)
func support_placement(index: int,count: int,who: int) -> Dictionary:
 var bounds=support_bounds();var margin=0.05;var gap=0.08
 var columns=1;var scale=0.0
 for candidate in range(1,count+1):
  var rows=ceili(float(count)/candidate)
  var fit=minf(FIELD_SCALE,minf((bounds.size.x-2*margin-(candidate-1)*gap)/(CARD_SIZE.y*candidate),(bounds.size.y-2*margin-(rows-1)*gap)/(CARD_SIZE.x*rows)))
  if fit>scale+0.001:columns=candidate;scale=fit
 # Large groups form two shallow fans so every card keeps a clickable edge.
 if scale<0.45:
  columns=ceili(float(count)/2.0);scale=0.45
 var rows=ceili(float(count)/columns)
 var row=int(index/columns);var column=index%columns
 var row_count=mini(columns,count-row*columns)
 var card_width=CARD_SIZE.y*scale;var card_depth=CARD_SIZE.x*scale
 var x_step=minf(card_width+gap,(bounds.size.x-2*margin-card_width)/maxi(1,columns-1))
 var row_width=card_width+(row_count-1)*x_step
 var x=bounds.position.x+(bounds.size.x-row_width)*0.5+card_width*0.5+column*x_step
 var z=bounds.position.y+(bounds.size.y-(rows*card_depth+(rows-1)*gap))*0.5+card_depth*0.5+row*(card_depth+gap)
 var side=1.0 if who==local_seat else -1.0
 return {"at":Vector3(x*side,0.035+index*0.001,z*side),"scale":scale,"fanned":x_step<card_width}
func field_position(group: String,index: int,count: int,who: int) -> Vector3:
 ...
 if wide_playmat:
  match group:
   ...
   "item":at=Vector3(-7.55+(index%4)*2.4,0.035+(index/4)*0.025,3.23+FIELD_EXTENSION)
...
func sync(payment_reservations: Array=[], selection: Array=[], available: Array=[], animate: bool=true):
 ...
 if animate and duel.combat.get("damage_batch",0)>0 and duel.combat.damage_batch!=last_damage_batch and visuals.has("card_"+str(duel.combat.attacker.uid)):
  last_damage_batch=duel.combat.damage_batch
  deferred_sync={"reserved":payment_reservations.duplicate(true),"chosen":selection.duplicate(),"available":available.duplicate()}
 ...
 for key in next:
  ...
  outline.material_override=material("#f3d397" if d.gold else "#69cce4")
  var halo=node.get_node("Halo")
  halo.visible=outline.visible
  halo.material_override=material("#803d38" if d.gold else "#205a83")
  ...
  if fresh or prev.get("at")!=d.at or prev.get("rotation")!=d.rotation or prev.get("scale")!=d.scale:
   move_card(key,node,d,animate and not presented_moves.has(d.uid))
 ...
 for key in visuals.keys():
  ...
  if animate and not presented_moves.has(d.uid):
   ...
   for group_key in next:
    if d.uid in next[group_key].get("members",[]):dest=next[group_key].at;break
...
func create_visual(d: Dictionary) -> Node3D:
 ...
 plane(root,Vector3(0,-0.015,0),CARD_SIZE+Vector2(0.48,0.48),material("#803d38")).name="Halo"
 plane(root,Vector3(0,-0.009,0),CARD_SIZE+Vector2(0.27,0.27),material("#f3d397")).name="Outline"
...
func sync_color_hexes(node: Node3D,d: Dictionary):
 var colors=[]
 if d.uid>0 and d.zone in ["field","palette"] and duel.cards.has(d.card_id):
  if d.zone=="palette" or duel.cards[d.card_id].kind!="符卡": colors=duel.Pack.colors(duel,duel.find_card(d.uid)) if not duel.find_card(d.uid).is_empty() else duel.cards[d.card_id].colors
 var signature=str(colors)
 if node.get_meta("hex_signature","")==signature: return
 node.set_meta("hex_signature",signature)
 if node.has_node("ColorHexes"):
  var old=node.get_node("ColorHexes"); node.remove_child(old); old.queue_free()
 var group=Node3D.new(); group.name="ColorHexes"; node.add_child(group)
 var ink={"红":"#ed3434","蓝":"#3188ff","绿":"#27ba47","黄":"#ffdf24","黑":"#141414"}
 var width=CARD_SIZE.x/5.0
 for i in range(colors.size()):
  var mesh=ImmediateMesh.new(); mesh.surface_begin(Mesh.PRIMITIVE_TRIANGLES)
  var points=[]
  for corner in range(6):
   var angle=TAU*corner/6.0-PI/2
   points.append(Vector3(sin(angle)*width/2,0,cos(angle)*width/2))
  for corner in range(6):
   mesh.surface_add_vertex(Vector3.ZERO); mesh.surface_add_vertex(points[corner]); mesh.surface_add_vertex(points[(corner+1)%6])
  mesh.surface_end()
  var hex=MeshInstance3D.new(); hex.mesh=mesh; hex.material_override=material(ink[colors[i]])
  hex.position=Vector3(-CARD_SIZE.x/2+width/2+i*width,0.06,-CARD_SIZE.y/2)
  group.add_child(hex)
...
func card_at(point: Vector2) -> int:
 ...
 var ray=PhysicsRayQueryParameters3D.create(origin,origin+camera.project_ray_normal(point)*100)
 ray.collide_with_areas=true; ray.collide_with_bodies=false
 var result=get_world_3d().direct_space_state.intersect_ray(ray)
 if result.is_empty() or result.collider.get_meta("kind","") not in ["object","stack"]: return 0
 if result.collider.get_meta("kind","")=="stack":
  var id=result.collider.get_meta("stack_id",-1)
  if not duel.stack.any(func(entry): return entry.id==id and entry.kind=="card"): return 0
 return result.collider.get_meta("uid",0)

func debug_drop_zone(point: Vector2,owner: int) -> String:
 var ray_origin=camera.project_ray_origin(point)
 var intersection=Plane(Vector3.UP,0).intersects_ray(ray_origin,camera.project_ray_normal(point))
 if intersection==null: return ""
 var at: Vector3=intersection
 # Explicit mat slots take precedence over the broad battlefield rectangle.
 ...
 if at.x>=-7.5 and at.x<=7.5:
  if depth>=5.65 and depth<=7.95: return "palette"
  if depth>=0.1 and depth<5.65: return "field"
 return ""

func debug_field_group(point: Vector2) -> Dictionary:
 var origin=camera.project_ray_origin(point)
 var intersection=Plane(Vector3.UP,0).intersects_ray(origin,camera.project_ray_normal(point))
 if intersection==null:return {}
 var at: Vector3=intersection
 for who in range(2):
  var side=1.0 if who==local_seat else -1.0
  var position=Vector2(at.x*side,at.z*side)
  if wide_playmat:
   for group in ["unit","item","support"]:
    if WIDE_ZONES[group].has_point(position):return {"owner":who,"group":group}
  else:
   var split=2.4 if top_down_view else 3.1
   if Rect2(-7.5,0.1,15.0,split-0.1).has_point(position):return {"owner":who,"group":"unit"}
   if Rect2(-7.5,split,8.0,5.65-split).has_point(position):return {"owner":who,"group":"item"}
   if Rect2(0.5,split,7.0,5.65-split).has_point(position):return {"owner":who,"group":"support"}
 return {}
...
func animate_combat_resolution():
 ...
 damage_revealed=false
 combat_display_cards=duel.combat.get("damage_snapshot",{}).duplicate(true)
 ...
 animation_count+=1
 sequence.tween_callback(func(): damage_revealed=true; combat_damage_shown.emit())
 sequence.tween_interval(damage_hold_seconds)
