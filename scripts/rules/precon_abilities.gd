...
static func event(e,c: Dictionary,effect: String,optional: bool=false,data: Dictionary={}):
 ...
 e.Cat.enqueue(e,{"precon":true,"extended":true,"owner":c.owner,"source":c.duplicate(true),"effect":effect,"optional":optional,"data":data.duplicate(true),"name":e.cards[c.card_id].name,"ability_text":text(e.cards[c.card_id],effect)})
...
static func response_locked(e,who: int=-1) -> bool:
 ...
 return e.players[who].get("night_lock",-1)==e.turn or e.stack.any(func(s):return s.kind=="card" and e.Cat.has(e,s.card,"spell-fdn-008")) or trigger_locked(e) or e.stack.any(func(s):return s.kind=="card" and s.owner!=who and e.Roster.has(e.cards[s.card.card_id],"gungnir"))
...
static func choice_valid(e,options: Array,t: Dictionary) -> bool:
 ...
 for spec in options:
  ...
  for field in ["x","ignore_color","pitch","counter_payment","n21_modes"]:
...
static func has_character(e,who: int,name: String) -> bool:
 return e.units(who).any(func(c): return e.Roster.character_matches(e.cards[c.card_id].character,name))
...
static func field_return(e,t: Dictionary,who: int=-1):
 ...
 e.detach(c);c.motion_from_owner=c.owner; c.owner=controller; e.enter_field(c,controller)
 ...
 match t.effect:
  ...
  "tewi_counters":
   for r in picked(target):
    if e.target_valid(r,true): var u=e.find_card(r.uid); e.Roster.plus(e,u,1,who)
  ...
  "sanae_end":
   if e.target_valid(target,true): var u=e.find_card(target.uid); e.Roster.plus(e,u,1,who)
  "tenshi_tap":
...
static func spell_resolve(e,entry: Dictionary) -> bool:
 ...
 match k:
  ...
  "spring":
   if e.target_valid(t,true):
    var c=e.find_card(t.uid); e.Roster.Batch.color_counter(e,c,t.color,who); e.draw(who)
...
static func activation_error(e,c: Dictionary,k: String) -> String:
 ...
 if k in ["standing_blast","watch_counter","letty_shield","wine_discount","medicine_return"] and not e.can_sacrifice(c):return "不能牺牲该永久物"
...
static func pay_activation(e,c: Dictionary,k: String,t: Dictionary):
 ...
 if k in ["standing_blast","watch_counter","letty_shield","wine_discount","medicine_return"]: e.sacrifice(c)
 if k=="keine_devour":
...
static func adjusted_damage(e,t: Dictionary,amount: int) -> int:
 ...
 for w in wards.duplicate():
  if amount<=0: break
  if w.turn not in [-1,e.turn]: wards.erase(w); continue
...
static func on_damage(e,t: Dictionary,amount: int):
 ...
 var info=e.cards[source.card_id]
 if e.Extra.keyword(e,source,"追击") and source.owner==e.active and e.usage_count(source.uid,"chase")==0:
...
static func ai_target(e,who: int,options: Array,effect: String="") -> Dictionary:
 ...
 if not specs.is_empty():
  var spec=specs[0]; var result=spec.duplicate(true);result.erase("selection");result.picks=[];var seen=[]
  for g in spec.selection:
   ...
   for r in pool:
    ...
    if g.has("sum_max") and list.reduce(func(n,p):return n+int(p.get("choice_weight",0)),0)+int(r.get("choice_weight",0))>g.sum_max:continue
    if g.get("distinct_names",false):
     var name=e.cards[e.find_card(r.uid).card_id].name
  ...
  if choice_valid(e,options,result):return result
  for option in options:
   if option.get("copy_back",false):return option
  return {}
