...
static func text(info: Dictionary,k: String) -> String:
 if New.CAPTIONS.has(k):return New.CAPTIONS[k]
...
static func pick(e,pool: Array,minimum: int,maximum: int,title: String,tag: String="",distinct: bool=false) -> Array:
 if "牺牲" in title:pool=pool.filter(func(r):return r.has("uid") and e.can_sacrifice(e.find_card(r.uid)))
 if distinct:
...
static func stat_adjust(e,c: Dictionary,k: String) -> int:
 ...
 var n=Cat.State.stat_adjust(e,c,k)+New.stat(e,c,k)
...
static func spell_options(e,id: String,who: int) -> Variant:
 var fresh=New.spell_options(e,id,who)
...
static func trigger_options(e,t: Dictionary) -> Array:
 var fresh=New.trigger_options(e,t)
 if fresh!=null:return fresh
...
static func spell_resolve(e,entry: Dictionary) -> bool:
 if New.spell_resolve(e,entry):return true
 ...
 match k:
  ...
  "gather_counters":
   ...
   for u in e.units(0)+e.units(1):
    for counter_key in Cat.BOOST_COUNTERS:
     total+=int(u.get(counter_key,0));u[counter_key]=0
   # CR 1.22.2: the recipient may die between removal and placement.
   e.judge()
 ...
 if k=="history_exile" and c.zone=="exile":c.history_spent=true
 return true

static func activation_cost(k: String) -> Dictionary:
 if k in New.ACTIVATIONS:return New.activation_cost(k)
...
static func activation_error(e,c: Dictionary,k: String) -> String:
 if k in New.ACTIVATIONS:return New.activation_error(e,c,k)
...
static func activation_options(e,c: Dictionary,k: String) -> Array:
 if k in New.ACTIVATIONS:return New.activation_options(e,c,k)
 ...
 match k:
  ...
  "kanako_blast":
   var g=e.Pack.group(refs(e,e.units(who).filter(func(u):return e.cards[u.card_id].character=="御柱" and e.can_sacrifice(u))),1,1,"牺牲一个御柱");g.cost=true
   return e.Pack.selection([g,e.Pack.group(e.Pack.all_units(e),1,1,"目标单位")],k)
  "alice_recycle":
   var g=e.Pack.group(e.Pack.all_units(e,who).filter(func(r):return r.uid!=c.uid and e.can_sacrifice(e.find_card(r.uid))),1,1,"牺牲另一个单位");g.cost=true
...
static func resolve_activation(e,entry: Dictionary):
 if entry.effect in New.ACTIVATIONS:New.resolve_activation(e,entry);return
