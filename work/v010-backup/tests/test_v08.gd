...
func run():
 ...
 view.auto_pay=true; view.request_cast(buff.uid); view.choose_target(e.ref_target(other)); view.confirm_declaration()
 expect(not view.modal and view.local.mode=="payment_offer" and view.observe_button.visible,"inline payment decision includes observation toggle")
 var before=snapshot(); var local_before=view.local.duplicate(true); var root_id=0
 await press("观察战场")
 expect(view.observing and not find_button(view.ui,"自动支付").visible,"observation hides inline payment controls")
 ...
 await press("返回选择")
 expect(view.local==local_before and find_button(view.ui,"自动支付").visible,"same inline payment choice is restored with local data")
 await press("手动支付")
 ...
 expect(panel.get_theme_stylebox("panel") is StyleBoxEmpty and panel.find_children("*","TextureRect",true,false).filter(func(c): return c.has_meta("damage_card")).size()==2,"three blockers display exactly two frameless card faces")
