...
func run():
 ...
 expect(Database.last_error.is_empty() and cards.size()==65,"registered database loads with no missing-art error")
 expect(Database.IDS==migration.ids,"no pending new cards were registered")
 if cards.size()!=65: quit(1); return
 ...
 app.load_test_decks(); app.editor(); app.selected="177"; app.update_preview(); await process_frame
