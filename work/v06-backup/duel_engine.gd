...
func shift(c: Dictionary, location: String):
 c.zone=location; c.epoch+=1; c.tapped=false; c.damage=0; c.timer=0; c.attacked=false
...
func payment_search(sources: Array,index: int,needs: Array) -> Dictionary:
 ...
 return best
func payment(who: int, cost: Dictionary) -> Dictionary:
 payment_memo.clear()
 var needs=[]
 for color in COLORS: needs.append(int(cost.get(color,0)))
 return payment_search(source_resources(who),0,needs)
...
func enter_field(c: Dictionary,who: int) -> bool:
 ...
 shift(c,"field"); c.entered=turn; players[who].field.append(c)
 return true
...
func judge():
 for who in range(2):
  for c in players[who].field.duplicate():
   if is_unit(c) and c.damage>=cards[c.card_id].health:
...
func finish_turn():
 for p in players:
  for c in p.field: c.damage=0
 start_turn(1-active)
func can_attack(who: int,uid: int) -> bool:
 var c=find_card(uid)
 return winner==-2 and pending.is_empty() and priority==who and active==who and phase=="main" and stack.is_empty() and combat.is_empty() and not c.is_empty() and c.owner==who and c.zone=="field" and is_unit(c) and not c.tapped and c.entered<turn
...
func advance_combat():
 ...
 match combat.step:
  ...
  "block_window":
   if surviving.size()>1:
    pending={"kind":"damage_assignment","owner":combat.owner,"total":cards[find_card(combat.attacker.uid).card_id].power}
   else: combat_damage({})
...
func combat_damage(allocation: Dictionary):
 ...
 var attacker=find_card(combat.attacker.uid)
 if not target_valid(combat.attacker,true): end_combat(); return
 var power=cards[attacker.card_id].power
 ...
 pending={}
 if not combat.blocked: players[1-combat.owner].life-=cards[attacker.card_id].spirit
 else:
  var retaliation=0
  for t in combat.blockers:
   var blocker=find_card(t.uid)
   blocker.damage+=power if combat.blockers.size()==1 else int(allocation.get(str(t.uid),0))
   retaliation+=cards[blocker.card_id].power
...
func ai_step(who: int=1):
 ...
 if not pending.is_empty():
  ...
  match pending.kind:
   ...
   "damage_assignment":
    ...
    for t in combat.blockers:
     var c=find_card(t.uid); var amount=mini(left,cards[c.card_id].health-c.damage)
 ...
 if active==who and phase=="main" and combat.is_empty():
  ...
  for c in choices:
   ...
   commit_cast(who,c.uid,target,payment(who,cards[c.card_id].cost).plan); return
  for c in units(who):
