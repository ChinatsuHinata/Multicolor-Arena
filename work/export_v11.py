...
try:
 ...
 with (work/'export.log').open('wb') as log:
  proc=subprocess.run([exe,'--headless','--path',str(root),'--export-release','Windows Desktop',str(root/'builds/Windows-1.1/MulticolorArena.exe'),'--log-file',str(work/'export-engine.log')],env=env,stdout=log,stderr=subprocess.STDOUT,timeout=300,creationflags=subprocess.CREATE_NO_WINDOW)
...
for p in (root/'builds/Windows-1.1').iterdir():
