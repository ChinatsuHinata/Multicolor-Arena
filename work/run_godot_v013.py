...
for name in sys.argv[1:]:
 ...
 label=Path(path).stem;log=root/'work/v013'/f'{label}.log'
 if visual:log=root/'work/v013'/f'{label}-visual.log'
...
(root/'work/v013/last-results.json').write_text(json.dumps(results,ensure_ascii=False,indent=2),encoding='utf-8')
