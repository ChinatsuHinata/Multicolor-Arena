...
func run():
 ...
 app.load_legacy_test_decks(); app.begin_battle(true)
 ...
 expect(view.local.get("uid",-1)==fire.uid and snapshot()==before,"plain hand click privately prepares casting")
 await click(Vector2(1050,700),MOUSE_BUTTON_RIGHT)
 expect(view.local.is_empty() and snapshot()==before,"right click outside cancels private hand declaration")
 await click(hand_point)
 expect(view.local.get("uid",-1)==fire.uid and view.local.get("mode","")=="target","click prepares card without dragging")
 expect(snapshot()==before,"click preview does not change public game")
 ...
 view.table.animation_duration=0.015; view.table.damage_hold_seconds=0.015; view.response_mode=view.ResponseMode.DEFAULT
 var frames=0
