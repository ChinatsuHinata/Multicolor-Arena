...
func layout() -> Dictionary:
 ...
 for who in range(2):
  ...
  for c in p.field:
   var d
   if duel.is_unit(c):
    var at=Vector3(-6.2+unit_index*2.46,0.035,1.65*side)
    d=description(c,at,0.95); unit_index+=1
   else:
...
func sync(payment_reservations: Array=[], selection: Array=[], available: Array=[], animate: bool=true):
 ...
 for key in next:
  ...
  var can_possess=duel.pending.get("kind","")=="possession" and duel.pending.get("owner",-1)==0 and d.owner==0 and d.zone=="palette" and d.uid>0 and not duel.find_card(d.uid).tapped
