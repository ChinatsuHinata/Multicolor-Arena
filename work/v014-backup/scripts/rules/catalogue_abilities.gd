...
static func printed_token(e,who,id):
 var c=e.make_card(id,who,"token");return c if e.enter_field(c,who) else {}
...
static func resolve_delay(e,d):
 ...
 return true
static func reveal(e,list,from_top=true):
 for c in list:
  e.record_history("展示 · "+e.cards[c.card_id].name,e.history_art(c))
...
static func counter_refs(e,list):
 var result=[]
 for c in list:
  for k in ["plus_counters","minus_counters","leader_counters","courage","scare","poverty","timer","dream","madness"]:
...
static func copy_token(e,who,template):
 ...
 var c=e.make_card(id,who,"token");return c if e.enter_field(c,who) else {}
static func copy_spell(e,t,x=-1):
