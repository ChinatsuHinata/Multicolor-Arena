...
func update_inspection():
 ...
 var signature=inspect_id+str(inspect_uid)+inspect_caption+str(enabled)+str(current.get("courage",0))+str(current.get("moods",[]))
 ...
 if current.get("courage",0)>0:inspection_text.add_text("\n勇气等级：%d" % current.courage)
...
func browse_zone(who: int,zone: String,scroll_position: int=0):
 ...
 for c in cards:
  ...
  var art=host.card(tile,"back" if card_hidden else c.card_id,Rect2(0,0,83,116)); art.name="PileCard"
