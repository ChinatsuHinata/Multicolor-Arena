...
static func resolve_trigger(e,t) -> bool:
 ...
 match t.effect:
  ...
  "larva_feed":
   var n=0
   for r in e.Pack.picked(aim):
    if e.Roster.unit(e,r):e.move_to(e.find_card(r.uid),"grave");n+=1
 ...
 match k:
  ...
  "peach_modes":
   if aim.mode=="回复并抓牌":e.gain_life(who,3);e.draw(who)
    ...
    var amounts={}
    for key in ["plus_counters","minus_counters","courage","leader_counters","scare","poverty","timer"]:amounts[key]=int(u.get(key,0))
...
static func rewritten_resolve(e,entry):
 ...
 e.enter_field(e.make_card(id,entry.owner,"token"),entry.owner);e.move_to(entry.card,"grave")
...
static func activation_options(e,c,k) -> Array:
 match k:
  ...
  "mask_sorrow":
   var result=[]
   for who in range(2):
    var choices=e.Roster.pick(e,e.Pack.zone(e,who,"palette"),0,2,("你" if who==0 else "人机")+"的颜色盘：至多两张","sorrow_"+str(who))
...
static func pay_activation(e,c,k):
 ...
 if k in ["mask_joy","mask_anger","mask_sorrow"]:e.move_to(c,"grave")
