...
func shift(c: Dictionary, location: String):
 ...
 for k in ["courage","moods","minus_counters","scare","freeze_until","perfect_lock","control_return","died_turn","excel_access"]:c.erase(k)
...
func cast_error(who: int,uid: int) -> String:
 ...
 if not info.requires_character.is_empty() and not Roster.bypass(self,c,who):
  ...
  if "支援" in info.get("keywords",[]) and players[who].leader.zone=="leader" and Roster.character_matches(cards[players[who].leader.card_id].character,info.requires_character): present=true
...
func commit_cast(who: int,uid: int,target: Dictionary,plan: Array) -> String:
 ...
 if info.kind=="符卡" or not info.get("variable_cost","").is_empty():
 ...
 for reservation in plan:
  ...
  else: tap_card(find_card(reservation.uid))
...
func choose_return(yes: bool):
 ...
 pending={}
 if yes:
  shift(c,"leader"); add_timer(c,0 if Roster.enabled(self,before,"cirno_return") else 2)
  note("自机返回自机区 · 计时 2")
 else:
  ...
  if blink_owner>=0 and destination=="exile":Roster.field_many(self,[c],blink_owner)
 ...
 match phase:
  ...
  "main":
   ...
   for c in units(active):
    ...
    if c.attacked and (DB.has_ability(cards[c.card_id],"brave") or Extra.keyword(self,c,"英勇")) and not Pack.response_locked(self):
...
func attack(who: int,uid: int,target: Dictionary={}):
...
func legal_blockers() -> Array:
 ...
 for c in units(1-combat.owner):
  ...
  if (DB.has_ability(cards[attacker.card_id],"exterminate") and has_leader_ability(attacker) or Extra.keyword(self,attacker,"退治")) and "人类" not in cards[c.card_id].race: continue
  result.append(c)
...
func block(uids: Array):
 ...
 for c in chosen: tap_card(c); combat.blockers.append(ref_target(c)); Pack.on_attack_or_block(self,c,false)
...
func move_to(c: Dictionary,zone: String,offer_return: bool=true,allow_replacement: bool=true):
 ...
 if allow_replacement and c.zone=="grave" and zone=="hand" and not grave_replacement_sources(c).is_empty():
  ...
  return
 if zone=="grave" and c.get("exile_on_grave",false):zone="exile"
 ...
 if pending.has("stack_id"):
  for entry in stack:
   if entry.id==pending.stack_id:
    entry.target=target.duplicate(true); entry.erase("awaiting_target");Roster.on_ability_announced(self,entry); return
...
func run_delayed(at_phase: String):
 for d in delayed.duplicate():
  ...
  Extra.event(self,source,"token_sacrifice" if d.effect=="token_sacrifice" else "delayed_return",false,d)
...
func commit_extension(who: int,uid: int,target: Dictionary,plan: Array,key: String="") -> String:
 ...
 for item in plan:
  ...
  else: tap_card(find_card(item.uid))
 var source=c.duplicate(true)
 ...
 Roster.on_ability_announced(self,stack.back())
