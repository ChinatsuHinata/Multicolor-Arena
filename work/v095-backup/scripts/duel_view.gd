...
var browser_cards: VBoxContainer
...
func begin(parent,a: Dictionary,b: Dictionary,first: int,seed_value: int=0):
 ...
 if debug_mode:
  ...
  btn("调试",Rect2(1460,57,116,34),debug_menu,false,debug_controls)
  btn("你的牌库",Rect2(1185,57,125,34),func(): browse_zone(0,"deck"),false,debug_controls)
  btn("人机牌库",Rect2(1320,57,125,34),func(): browse_zone(1,"deck"),false,debug_controls)
...
func stage_input(event: InputEvent):
 if (modal and not observing) or dragging or debug_drag_uid!=0: return
...
func render():
 ...
 life_widgets={}
 render_life(1,Rect2(1350,99,214,61))
 ...
 elif engine.pending.get("kind","")=="trigger_order" and engine.pending.owner==acting_player(): trigger_order_menu()
 refresh_observation()
 ...
 for key in card_badges:
  ...
  if table.visuals.has(key):
   ...
   if parts.has("caption"):
    root.position=Vector2(clampf(rect.get_center().x,1065,1460),rect.end.y+6)
...
func trigger_order_menu():
 ...
 panel.position=Vector2(850,185); panel.size=Vector2(728,495)
 ...
 for i in range(options.size()):
  var t=options[i]; var at=Vector2(12+(i%3)*222,(i/3)*335)
...
func commit_local():
 ...
 if error.is_empty(): local={}; selection=[]; picker.reset(); message=""
 else: message=error
 modal=false; render()
func cancel_cast():
 local={}; selection=[]; picker.reset(); modal=false; message=""; render()
func overlay(title: String) -> Panel:
 ...
 modal_root=shade
 var panel=host.box(shade,Rect2(920,240,650,330),Color("#101c28"),host.GOLD)
...
func damage_dialog():
 ...
 var width=728.0
 panel.position=Vector2(850,170); panel.size=Vector2(width,650)
...
func choose_options(title: String,options: Array,callback: Callable):
 ...
 panel.position=Vector2(850,155); panel.size=Vector2(728,590)
...
func open_deck_casts():
 var options=engine.legal_casts(acting_player()).filter(func(c): return c.zone=="deck").map(func(c): return engine.ref_target(c))
 choose_options("牌库可用牌",options,func(t): request_cast(t.uid))
func close_debug():
...
func browse_zone(who: int,zone: String,scroll_position: int=0):
 ...
 debug_root=layer(ui)
 browser_panel=host.box(debug_root,Rect2(1288,143,294,741),Color("#101c28"),Color("#637a93"))
 browser_panel.name="PileBrowser"
 var cards=zone_cards(who,zone)
 txt(("你" if who==0 else "人机")+" · "+ZONE_NAMES[zone]+"  %d" % cards.size(),Rect2(15,10,223,38),21,host.GOLD,browser_panel)
 btn("×",Rect2(245,10,37,36),close_debug,false,browser_panel)
 var top=58
 if debug_mode:
  var owner_picker=OptionButton.new(); owner_picker.position=Vector2(13,55); owner_picker.size=Vector2(83,37); browser_panel.add_child(owner_picker)
  ...
  var zone_picker=OptionButton.new(); zone_picker.position=Vector2(102,55); zone_picker.size=Vector2(177,37); browser_panel.add_child(zone_picker)
  ...
  top=104
 browser_scroll=ScrollContainer.new(); browser_scroll.name="PileScroll"; browser_scroll.position=Vector2(14,top); browser_scroll.size=Vector2(268,726-top)
 browser_scroll.horizontal_scroll_mode=ScrollContainer.SCROLL_MODE_DISABLED; browser_panel.add_child(browser_scroll)
 browser_cards=VBoxContainer.new(); browser_cards.name="PileCards"; browser_cards.size_flags_horizontal=Control.SIZE_EXPAND_FILL
 browser_cards.add_theme_constant_override("separation",16); browser_scroll.add_child(browser_cards)
 var hidden=zone=="deck" and not debug_mode
 for i in range(cards.size()):
  var c=cards[i]
  var tile=Control.new(); tile.custom_minimum_size=Vector2(245,379); browser_cards.add_child(tile)
  tile.set_meta("display_id","back" if hidden else c.card_id)
  var art=host.card(tile,"back" if hidden else c.card_id,Rect2(6,0,232,324))
  art.name="PileCard"; art.mouse_default_cursor_shape=Control.CURSOR_DRAG if debug_mode else Control.CURSOR_POINTING_HAND
  art.gui_input.connect(func(event):
   if event is InputEventMouseButton and event.pressed:
    if event.button_index==MOUSE_BUTTON_RIGHT or (event.button_index==MOUSE_BUTTON_LEFT and not debug_mode):
     inspect_card("back",0,"牌库 · 未公开") if hidden else inspect_card(c.card_id,c.uid)
    elif event.button_index==MOUSE_BUTTON_LEFT: begin_debug_drag(c.uid,art.get_global_transform()*event.position))
  var title=str(i+1) if hidden else "%d  %s" % [i+1,engine.cards[c.card_id].name]
  var caption=txt(title,Rect2(4,329,235,48),16,host.MUTED if hidden else host.WHITE,tile)
  caption.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART; caption.horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER
  if not hidden and c.owner==acting_player() and not response_disabled() and engine.pending.is_empty():
   ...
   if zone=="grave" and not action.is_empty() and engine.extension_activation_error(c.owner,c).is_empty():
    tile.custom_minimum_size.y=424
    btn("发动",Rect2(67,380,110,38),func(): close_debug(); execute_action(action),true,tile)
 if cards.is_empty(): txt("空",Rect2(110,top+50,60,35),22,host.MUTED,browser_panel)
 browser_scroll.set_deferred("scroll_vertical",scroll_position)
 refresh_observation()
func debug_menu():
 if debug_mode: browse_zone(acting_player(),"deck")
...
func handle_debug_drag(event: InputEvent):
 if event is InputEventMouseMotion:
  ...
  if not debug_dragging and debug_drag_pointer.distance_to(debug_drag_origin)>10:
   ...
   debug_drop_hint.position=Vector2(-15,199); debug_drop_hint.horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER
  ...
  if debug_dragging:
   ...
   debug_drop_hint.text=ZONE_NAMES.get(destination,"取消移动")
...
func finish_debug_drag(cancelled: bool):
 ...
 if cancelled or not moved or c.is_empty() or c.epoch!=debug_drag_epoch: return
 ...
 render()
 browse_zone(browser_owner,browser_zone,scroll_position)
...
func render_inline_picker(confirm: Callable,optional: bool=false):
 ...
 for atom in picker.available():
  ...
  if atom.value.has("uid"):
   var c=engine.find_card(atom.value.uid)
   if not c.is_empty() and c.zone not in ["field","palette","leader","stack","hand"]: options.append(atom)
