extends RefCounted
# Explicit handlers for the registered card pool; text alone never executes effects.
...
static func resolved(engine, entry: Dictionary):
 ...
 if card.kind!="符卡":
  engine.enter_field(entry.card,entry.owner)
  return
 if engine.Extra.spell_resolve(engine,entry): return
