...
static func active_save_path() -> String:
 return SAVE_PATH if OS.has_feature("editor") else USER_SAVE_PATH
...
static func load_decks(path: String = "") -> Dictionary:
 var first_run=path.is_empty() and not OS.has_feature("editor")
 if path.is_empty(): path=active_save_path()
 if first_run and not FileAccess.file_exists(path):
  if not FileAccess.file_exists(BUNDLED_DECKS_PATH):
   return {"error":"发行包缺少初始卡组，请重新导出。", "decks":[]}
  var bundled=load_decks(BUNDLED_DECKS_PATH)
  if bundled.has("error"): return bundled
  var save_error=persist(bundled.decks,path)
  if not save_error.is_empty(): bundled["error"]=save_error
  return bundled
