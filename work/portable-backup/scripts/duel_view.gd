...
func btn(text: String,rect: Rect2,action: Callable,accent: bool=false,parent: Node=null):
 ...
 var inspection_action=text in ["设置","对局记录","继续游戏","返回联机房间","观察战场","返回选择"]
...
func render():
 ...
 txt("连接中断，对局结束" if network_session!=null and network_session.ended() else "第 %d 回合  ·  %s  ·  %s" % [engine.turn,player_caption(engine.active),phase_names[engine.phase]],Rect2(310,12,790,42),23,host.GOLD)
 btn("设置",Rect2(1460,12,116,40),settings_menu)
 btn("对局记录",Rect2(1310,99,266,40),open_history)
 if network_session!=null:
  network_latency_label=txt(network_session.latency_text(),Rect2(1278,58,298,31),16,host.MUTED)
...
func render_prompt():
 if network_session!=null and not network_session.can_act(true):
  ...
  if network_session.ended():btn("返回联机房间",Rect2(1330,799,237,49),func():host.online(),true)
  return
 if table.combat_animating: return
 var text=""
 if not local.is_empty():
...
func network_locked() -> bool:
 return network_session!=null and (not network_session.connected or network_session.paused or network_session.ended())
