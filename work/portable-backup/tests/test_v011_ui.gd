...
func run():
 ...
 view.render();await settle();await click(view.hand_nodes[spell.uid].get_global_rect().get_center());await press("X = 2");view.choose_target(e.ref_target(unit));await press("确定")
 ...
 var atom=view.region_atoms().filter(func(a):return a.value.uid==grave.uid)[0];view.select_region_atom(atom);view.confirm_region_atom();await settle();await finish_group();await press("确定")
 if view.local.get("mode")=="payment_offer":await press("自动支付")
 ...
 var act=e.available_actions(0,satori.uid).filter(func(a):return a.get("key")=="satori_discard")[0];view.execute_action(act);view.choose_target({"player":1});await press("确定");resolve();view.render();await settle()
