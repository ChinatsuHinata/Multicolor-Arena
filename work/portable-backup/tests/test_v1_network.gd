...
func run():
 ...
 check(host.ended() and guest.ended() and host.room.status=="aborted" and guest.room.status=="aborted","both terminate entire series")
 check(host.room.winner==-2 and guest.room.winner==-2 and host.series.state.scores==scores and host.series.state.counted==counted,"disconnect termination awards no win or game result")
