...
func cast_error(who: int,uid: int) -> String:
 ...
 if not info.requires_character.is_empty():
  ...
  if "支援" in info.get("keywords",[]) and players[who].leader.zone=="leader" and cards[players[who].leader.card_id].character==info.requires_character: present=true
...
func stat(c: Dictionary,key: String) -> int:
 var value=Pack.base_stat(self,c,key)+int(c.get("plus_counters",0))
...
func combat_damage(allocation: Dictionary):
 ...
 if not combat.blocked:
  ...
  for t in combat.blockers:
   ...
   var counter_damage=combat_hit(blocker,combat.attacker,stat(blocker,"power")) if deals_combat_damage(blocker) else 0
...
func choose_effect(target: Dictionary):
 ...
 if not target.is_empty() and not Pack.choice_valid(self,Extra.trigger_options(self,t),target): return
