...
static func keyword(e,c: Dictionary,k: String) -> bool:
 ...
 return e.Pack.dynamic_keyword(e,c,k) or k in e.cards[c.card_id].get("keywords",[]) or c.get("modifiers",[]).any(func(m): return m.get(k,false))
static func cost_value(e,c: Dictionary) -> int:
 var n=int(c.get("cast_x",0)) if c.zone=="stack" else 0
...
static func event(e,c: Dictionary,effect: String,optional: bool=false,data: Dictionary={}):
 if e.Pack.response_locked(e): return
