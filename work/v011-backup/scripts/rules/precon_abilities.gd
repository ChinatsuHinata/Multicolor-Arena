...
static func response_locked(e) -> bool:
...
static func cost(e,c: Dictionary,who: int,base: Dictionary,target: Dictionary) -> Dictionary:
 ...
 if not info.get("variable_cost","").is_empty(): result[info.variable_cost]=result.get(info.variable_cost,0)+int(target.get("x",0))
 if has(info,"promise"):
...
static func resolve_trigger(e,t: Dictionary):
 ...
 match t.effect:
  ...
  "sand_add":
   if original and e.players[who].palette.size()>=8: c.plus_counters=c.get("plus_counters",0)+2
  "tewi_counters":
   for r in picked(target):
    if e.target_valid(r,true): var u=e.find_card(r.uid); u.plus_counters=u.get("plus_counters",0)+1
  ...
  "sanae_end":
   if e.target_valid(target,true): var u=e.find_card(target.uid); u.plus_counters=u.get("plus_counters",0)+1
  "tenshi_tap":
  ...
  "oni_attack":
   if original: c.plus_counters=c.get("plus_counters",0)+2
  ...
  "old_city_heal":
   var heads=e.rng.randi_range(0,1)==1; e.note("旧都 · "+("正面" if heads else "反面"))
...
static func on_damage(e,t: Dictionary,amount: int):
 ...
 if "吸血1" in info.get("keywords",[]) and e.usage_count(source.uid,"lifesteal_%d" % e.next_damage_batch)==0:
  e.use_once(source.uid,"lifesteal_%d" % e.next_damage_batch); e.gain_life(source.owner,1)
