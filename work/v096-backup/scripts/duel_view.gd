...
func _process(delta):
 ...
 if history_open or debug_open or observing or modal or not local.is_empty() or drag_uid!=0 or engine.winner!=-2: return
...
func render():
 ...
 var side_bg=host.box(hud,SIDEBAR,Color("#0d1825"),Color("#304657")); side_bg.mouse_filter=Control.MOUSE_FILTER_IGNORE
 for i in range(3):
 ...
 elif region_picker_needed(): region_picker()
 refresh_observation()
...
func open_deck_casts():
 var options=engine.legal_casts(acting_player()).filter(func(c): return c.zone=="deck")
 var panel=overlay("牌库可用牌"); panel.position=Vector2(800,190); panel.size=Vector2(775,490)
 var content=choice_card_content(panel,options.size(),Vector2(735,390))
 for i in range(options.size()):
  var c=options[i]; var at=Vector2(12+(i%3)*222,(i/3)*335)
  host.card(content,c.card_id,Rect2(at,Vector2(173,241)),func(): close_overlay(); request_cast(c.uid))
  var label=txt(engine.cards[c.card_id].name,Rect2(at+Vector2(0,250),Vector2(185,74)),17,host.GOLD,content)
  label.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART; label.size=Vector2(185,74)
func close_debug():
...
func browse_zone(who: int,zone: String,scroll_position: int=0):
 ...
 for i in range(cards.size()):
  ...
  var caption=txt(str(i+1)+" · "+("未公开" if hidden else engine.cards[c.card_id].name),Rect2(88,0,166,62),14,host.WHITE,tile)
  caption.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART; caption.size=Vector2(166,62)
  if not hidden:
   var rules=txt(engine.cards[c.card_id].rules_text,Rect2(88,64,166,48),13,host.MUTED,tile)
   rules.autowrap_mode=TextServer.AUTOWRAP_WORD_SMART; rules.size=Vector2(166,48)
...
func finish_debug_drag(cancelled: bool):
 ...
 if not moved: browse_card_action(c); return
