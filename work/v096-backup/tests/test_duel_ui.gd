...
func run():
 ...
 expect(view.auto_pay and view.response_mode==view.ResponseMode.DEFAULT,"payment defaults on and response mode defaults to smart")
 for who in range(2):
 ...
 view.fast_mode=true; view.response_mode=view.ResponseMode.ON; view.clock_time=2
 var rev=engine.revision
 view._process(1)
 expect(engine.revision==rev and engine.priority==0,"full response holds even without playable card")
 view.response_mode=view.ResponseMode.DEFAULT; view.clock_time=2; view._process(1)
