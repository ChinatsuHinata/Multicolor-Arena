...
func run():
 ...
 view.auto_pay=false; view.choose_target({"player":1})
 ...
 expect(not e.combat.is_empty() and e.combat.attacker.uid==unit.uid,"unit click directly declares its only enabled attack")
 ...
 view.full_response=false; view.render()
 ...
 view.full_response=true; e.priority=0; view.render()
 ...
 view.execute_action(actions[1]); view.choose_target({"player":1})
 ...
 view.table.animation_duration=0.015; view.full_response=false
 var frames=0
