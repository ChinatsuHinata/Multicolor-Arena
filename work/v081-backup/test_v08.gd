...
func run():
 ...
 expect(stack_face.get_node("Outline").visible and stack_face.get_node("Outline").material_override.albedo_color==Color("#ffd65c"),"yellow selected card stays yellow in stack")
 await capture("stack-yellow")
 ...
 expect(not view.modal and e.combat.get("attacker",{}).get("uid")==attacker.uid,"single attack action declares immediately without popup")
 ...
 view.object_clicked(attacker.uid); view.render(); await settle(); resolve(); e.block([]); view.render(); await settle()
