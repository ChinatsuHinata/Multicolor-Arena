...
func create_room(format_value: int=3,strict: bool=true,game_port: int=47861,interface_address: String="*") -> String:
 ...
 series.state.chooser=randi_range(0,1);series.state.first=series.state.chooser
...
func restore_host() -> String:
 ...
 disconnected_at=Time.get_ticks_msec();discovery.start(true,metadata(),bind_address)
 if authority!=null:enqueue_snapshot(make_snapshot(0,[],true))
 changed.emit();return ""
func persist() -> bool:
 if not is_host:return true
 var data={"fingerprint":fingerprint,"room_id":room_id,"port":port,"bind_address":bind_address,"guest":guest,"series":series.state,"sequence":sequence,"command_results":command_results,"receipts":receipts,"engine":Codec.capture(authority) if authority!=null else {}}
...
func leave(forget: bool=true):
 ...
 connected=false;paused=true;joining=false;remote_peer=0;room_id="";room={};applicant={};authority=null;busy=false;snapshots.clear();latest_snapshot={};local_game_id="";notice="";inflight={};disconnected_at=0;rejected=false
...
func accept_applicant(accept: bool):
 ...
 welcome(id)
func welcome(id: int):
 remote_peer=id;connected=true;paused=true;joining=false;remote_last_seen=Time.get_ticks_msec();disconnected_at=0;notice="同步中";retry_at=0
...
func receive(id: int,m: Dictionary):
 ...
 remote_last_seen=Time.get_ticks_msec()
 match type:
  "ping":transport.send_to(id,{"type":"pong"},true)
  "pong":pass
  ...
  "welcome":
   ...
   identity.resume={"room_id":room_id,"token":m.token,"match_id":m.match_id,"address":address,"port":port,"sequence":sequence};save_identity();changed.emit()
  "room":
   if is_host:return
  ...
  "ack":
   ...
   if int(m.get("sequence",-1))==sequence:
    acknowledged=sequence;paused=false;notice="";transport.send_to(id,{"type":"ready_state","sequence":sequence});changed.emit()
  "ready_state":
   if not is_host:paused=false;notice="";disconnected_at=0;changed.emit()
  "command":
...
func on_disconnect(id: int):
 if id!=remote_peer:return
 connected=false;paused=true;busy=false;joining=false
 if disconnected_at==0:disconnected_at=Time.get_ticks_msec()
 notice="连接中断，已暂停，等待原玩家重连";changed.emit()
func on_failure(message: String):
 if rejected:return
 connected=false;paused=true;busy=false;joining=false
 if disconnected_at==0 and not room_id.is_empty():disconnected_at=Time.get_ticks_msec()
 notice=message+"，对局已暂停";changed.emit()
func can_act(in_match: bool=false) -> bool:return connected and not paused and not busy and (not in_match or sequence==view_sequence)
...
func handle_room_action(actor: int,action: Dictionary):
 ...
 match name:
  ...
  "first":
   if series.state.status not in ["lobby","between"] or series.state.chooser!=actor or series.state.ready.any(func(v):return v):error="当前不能更改先后手"
   else:series.state.first=actor if action.get("first",true) else 1-actor;series.state.first_chosen=true
  "concede_series":
   series.state.status="complete";series.state.winner=1-actor
   if authority!=null and authority.winner==-2:authority.surrender(actor)
  _:return
...
func _process(_delta):
 ...
 if connected:
  if now-last_heartbeat>2000:last_heartbeat=now;transport.send_to(remote_peer,{"type":"ping"},true)
  if now-remote_last_seen>6500:on_disconnect(remote_peer);transport.drop(remote_peer)
 elif not is_host and not rejected and not identity.get("resume",{}).is_empty() and disconnected_at>0 and now-disconnected_at<120000 and now-retry_at>3000:
  retry_at=now;resume_guest()
 if not connected and disconnected_at>0 and now-disconnected_at>=120000 and notice!="重连等待超时，可手动恢复连接或离开":notice="重连等待超时，可手动恢复连接或离开";changed.emit()
