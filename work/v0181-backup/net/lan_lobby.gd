...
func build_home():
 ...
 address_input=input("",Rect2(625,690,620,49));address_input.placeholder_text="例如 192.168.1.20"
...
func build_room():
 ...
 if not session.applicant.is_empty():
  ...
  if room.chooser==own and not room.ready.any(func(v):return v):
   app.button(body,"先手"+(" ✓" if room.get("first_chosen",false) and room.first==own else ""),Rect2(105,552,310,49),func():session.room_action({"name":"first","first":true}),room.first==own)
   app.button(body,"后手"+(" ✓" if room.get("first_chosen",false) and room.first!=own else ""),Rect2(435,552,310,49),func():session.room_action({"name":"first","first":false}),room.first!=own)
  else:app.label(body,("先手："+room.names[room.first]) if room.get("first_chosen",false) else ("等待 "+room.names[room.chooser]+" 选择先后手"),Rect2(105,552,800,45),22)
  var ready=app.button(body,"已准备" if room.ready[own] else "准备",Rect2(1070,550,355,60),func():session.room_action({"name":"ready"}),true)
  ready.disabled=room.ready[own] or room.own_deck.is_empty() or not session.can_act() or not room.get("first_chosen",false)
 if not session.connected and not session.is_host:
  app.button(body,"重连",Rect2(1070,650,355,52),func():
   ...
   if not error.is_empty():show_error(error),true)
 if room.status in ["playing","between"]:
  app.button(body,"整场认输",Rect2(105,714,290,48),func():session.room_action({"name":"concede_series"}))
