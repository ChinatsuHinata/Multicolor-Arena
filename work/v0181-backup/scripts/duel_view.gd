...
func _process(delta):
 ...
 if network_session!=null:
  ...
  if is_instance_valid(network_status_label):
   network_status_label.text=network_session.notice if not network_session.can_act(true) else ""
...
func render():
 ...
 txt("第 %d 回合  ·  %s  ·  %s" % [engine.turn,player_caption(engine.active),phase_names[engine.phase]],Rect2(310,12,790,42),23,host.GOLD)
 btn("设置",Rect2(1460,12,116,40),settings_menu)
 btn("对局记录",Rect2(1310,99,266,40),open_history)
 if debug_mode:
  txt("操作："+player_caption(acting_player()),Rect2(970,15,158,32),18,host.GOLD)
 life_widgets={}
...
func sync_hand_nodes(who: int,nodes: Dictionary,parent: Control,available: Array):
 ...
 for i in range(cards.size()):
  ...
  var stride=minf(154,(HAND.size.x-80)/maxi(1,cards.size())) if who==local_seat else minf(74 if debug_mode else 52,750.0/maxi(1,cards.size()))
  var target=Vector2(HAND.position.x+15+i*stride,680) if who==local_seat else Vector2(STAGE.get_center().x-tile_size.x/2-(cards.size()-1)*stride/2+i*stride,64)
...
func update_inspection():
 ...
 image.texture=host.texture("back") if inspect_id=="back" else host.texture(inspect_id)
...
func render_prompt():
 if network_session!=null and not network_session.can_act(true):
  network_status_label=txt(network_session.notice if not network_session.notice.is_empty() else "同步中",Rect2(1330,738,237,70),18,host.GOLD)
  return
 if table.combat_animating: return
 var text=""
 if not local.is_empty():
...
func settings_menu():
 ...
 btn("投降",Rect2(330,180,260,48),func():
  ...
  btn("投降",Rect2(35,155,260,50),func(): modal=false; local={}; engine.surrender(local_seat); render(),true,confirm)
  btn("返回",Rect2(330,155,260,50),func(): modal=false; render(); settings_menu(),false,confirm),false,panel)
func result_overlay():
 var panel=overlay("平局" if engine.winner==-1 else "胜利" if engine.winner==local_seat else "对局结束")
 txt(engine.log.back(),Rect2(30,90,590,80),22,host.WHITE,panel)
 btn("返回对局准备",Rect2(160,220,330,56),func(): host.online() if network_session!=null else host.setup(),true,panel)
func damage_dialog():
...
func browse_zone(who: int,zone: String,scroll_position: int=0):
 ...
 if debug_mode:
  ...
  top=92
 browser_scroll=ScrollContainer.new(); browser_scroll.position=Vector2(8,top); browser_scroll.size=Vector2(SIDEBAR.size.x-16,SIDEBAR.size.y-top-8)
