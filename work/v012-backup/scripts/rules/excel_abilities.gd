...
static func event(e,c: Dictionary,k: String,optional: bool=false,data: Dictionary={}):
 ...
 e.triggers.append({"roster":true,"extended":true,"owner":c.owner,"source":c.duplicate(true),"effect":k,"optional":optional,"data":data.duplicate(true),"name":e.cards[c.card_id].name,"ability_text":text(e.cards[c.card_id],k)})
...
static func race(e,c: Dictionary,r: String) -> bool: return r in e.cards[c.card_id].race or "全部" in e.cards[c.card_id].race
...
static func field_many(e,list: Array,who: int,tapped: bool=false):
 ...
 var full=e.field_slots(who)+slots>6
 var entered=[]
 for c in eligible:
  ...
  e.detach(c); c.owner=who
  e.shift(c,"field");c.entered=e.turn;c.entered_turns=e.players[who].turns;c.tapped=tapped;e.players[who].field.append(c);entered.append(c)
 for c in entered:e.Extra.on_enter(e,c)
static func plus(e,c: Dictionary,n: int):
 if c.is_empty() or c.zone!="field" or n<=0: return
 c.plus_counters=int(c.get("plus_counters",0))+n
...
static func fast(e,c: Dictionary,who: int) -> bool:
 return (has(e.cards[c.card_id],"kogasa_flash") and not e.combat.is_empty() and e.combat.owner!=who) or forced(e,c,who)
...
static func bypass(e,c: Dictionary,who: int) -> bool:
 return forced(e,c,who) or permission(e,c,who) and c.get("excel_access",{}).get("ignore",false)
static func free_cast(e,c: Dictionary,who: int) -> bool:
 return forced(e,c,who) or permission(e,c,who) and c.zone=="exile" or e.players[who].field.any(func(u):return has(e.cards[u.card_id],"free_anthem"))
...
static func invalidate(e,t: Dictionary,who: int,spell: bool) -> Dictionary:
 ...
 elif protected(e,out,who,spell): out.uid=-1
 return out
...
static func spell_options(e,id: String,who: int) -> Variant:
 ...
 match k:
  ...
  "reset_four":options=pick(e,e.Pack.zone(e,who,"palette"),0,4,"重置至多四张颜色盘中的牌",k)
 ...
 if not e.cards[id].get("variable_cost","").is_empty():
  ...
  if not e.forced_cast.is_empty() or e.players[who].field.any(func(u):return has(e.cards[u.card_id],"free_anthem")):limit=0
...
static func on_enter(e,c: Dictionary):
 for k in ["sakuya_timer","orin_discard","miko_divide","parsee_catchup"]:
...
static func resolve_trigger(e,t: Dictionary):
 ...
 match t.effect:
  ...
  "kogasa_scare":
   if unit(e,aim):e.find_card(aim.uid).scare=int(e.find_card(aim.uid).get("scare",0))+1
   for p in range(2):
  ...
  "medicine_destroy":
   if unit(e,data.ref):e.destroy(e.find_card(data.ref.uid))
  "komachi_counters":
   if unit(e,data.ref):e.find_card(data.ref.uid).minus_counters=int(e.find_card(data.ref.uid).get("minus_counters",0))+3
  "komachi_coin":
  ...
  "chen_counter","akyuu_counter","shinmy_counter":
   if unit(e,aim):plus(e,e.find_card(aim.uid),1)
  "komachi_exile":
   var n=0
   for r in e.Pack.picked(aim):
  ...
  "search_take":
   ...
   for c in chosen:e.record_history("展示 · "+e.cards[c.card_id].name,e.history_art(c))
  ...
  "seiga_sacrifice":
   for r in e.Pack.picked(aim):
    if unit(e,r):e.move_to(e.find_card(r.uid),"grave")
  ...
  "koishi_coin":
   var heads=e.flip_coin(who);e.note("古明地恋 · "+("正面" if heads else "反面"))
   if same:
  ...
  "suwako_top","mist_lake":
   ...
   for r in top:
    var c=e.find_card(r.uid);e.record_history("展示 · "+e.cards[c.card_id].name,e.history_art(c))
...
static func sacrifice_others(e,keep: Array):
 ...
 for c in (e.units(0)+e.units(1)).duplicate():
  if not keep.any(func(r):return r.get("uid")==c.uid and r.get("epoch")==c.epoch):e.move_to(c,"grave")
 e.death_observers=[]
 ...
 match k:
  ...
  "reset_four":
   for r in e.Pack.picked(t):
    if valid(e,r):e.find_card(r.uid).tapped=false
  "color_damage":
   var colors=[]
  ...
  "leader_mark":
   if unit(e,t):var u=e.find_card(t.uid);u.leader_counters=int(u.get("leader_counters",0))+1
  "grave_top":
  ...
  "counters_x":
   if unit(e,t):plus(e,e.find_card(t.uid),x);e.apply_turn_buff(t,{"英勇":true})
  "unblock_reset":
  ...
  "gather_counters":
   ...
   if unit(e,t):plus(e,e.find_card(t.uid),total)
  "bounce_small":
   if unit(e,t):e.move_to(e.find_card(t.uid),"hand")
  "steal_turn":
   if unit(e,t):
    var u=e.find_card(t.uid);var prior=u.owner;e.players[prior].field.erase(u);u.owner=who;e.players[who].field.append(u)
  ...
  "door_reveal":
   ...
   for u in e.players[who].deck:
    top.append(e.Pack.ref(e,u));e.record_history("展示 · "+e.cards[u.card_id].name,e.history_art(u))
...
static func activation_error(e,c: Dictionary,k: String) -> String:
 ...
 if k=="sanae_search" and e.players[c.owner].get("sanae_used",false):return "本局已经发动"
 return ""
...
static func pay_activation(e,c: Dictionary,k: String,t: Dictionary):
 ...
 if k in ["alice_recycle","kanako_blast","sanae_search"]:
  for r in e.Pack.picked(t):e.move_to(e.find_card(r.uid),"grave")
 if k=="sanae_search":e.players[c.owner].sanae_used=true
...
static func cleanup(e):
 for c in (e.players[0].field+e.players[1].field).duplicate():
  ...
  if not control.is_empty():
   e.players[c.owner].field.erase(c);c.owner=control.owner;e.players[c.owner].field.append(c);c.erase("control_return");c.entered_turns=e.players[c.owner].turns
