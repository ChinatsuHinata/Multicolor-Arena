...
func pointer(event: InputEvent):
 ...
 if event.button_index in [MOUSE_BUTTON_WHEEL_UP,MOUSE_BUTTON_WHEEL_DOWN]:
  camera_distance=clampf(camera_distance+(-0.025 if event.button_index==MOUSE_BUTTON_WHEEL_UP else 0.025),0.77,1.18)
  set_camera(); return
...
func debug_drop_zone(point: Vector2,owner: int) -> String:
 ...
 if at.x>=-7.5 and at.x<=7.5:
  if depth>=4.65 and depth<=7.35: return "palette"
  if depth>=0.25 and depth<4.65: return "field"
 return ""
