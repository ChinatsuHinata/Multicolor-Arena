...
func next_card():
 ...
 if c.zone=="hand" and nodes.has(c.uid):
  ...
  hand_node.hide()
 create_face(initial_size,from,load("res://assets/card_back.svg"))
 ...
 sequence.tween_method(func(time:float):
  ...
  else:face.texture=load("res://assets/card_back.svg");face.scale.x=minf(1,(time-0.45)/0.025)
...
func zone_rect(zone: String,owner: int,uid: int,source: bool) -> Rect2:
 ...
 if source and zone=="hand":
  var nodes=view.hand_nodes if owner==0 else view.enemy_nodes
 ...
 if zone=="hand":
  ...
  var dimensions=Vector2(146,204) if owner==0 else Vector2(70,98) if view.debug_mode else Vector2(54,75)
  var stride=minf(154,(view.HAND.size.x-80)/maxi(1,cards.size())) if owner==0 else minf(74 if view.debug_mode else 52,750.0/maxi(1,cards.size()))
  var at=Vector2(view.HAND.position.x+15+index*stride,680) if owner==0 else Vector2(view.STAGE.get_center().x-dimensions.x/2-(cards.size()-1)*stride/2+index*stride,64)
...
func show_move():
 ...
 var hidden=current.from in ["hand","deck"] and current.to in ["hand","deck"] and (c.owner!=0 or current.from=="deck" and current.to=="deck") and not view.debug_mode
 var texture=load("res://assets/card_back.svg") if hidden else view.host.texture(c.card_id)
