...
func run():
 ...
 app.toggle_color("红")
 expect(app.selected_colors.is_empty() and app.library.get_child_count()==Store.CARDS.values().filter(func(c):return c.constructible).size(),"unchecking final color restores all")
 app.toggle_color("黑"); app.toggle_color("全部")
 expect(app.selected_colors.is_empty() and app.library.get_child_count()==Store.CARDS.values().filter(func(c):return c.constructible).size(),"all button clears strict filters")
