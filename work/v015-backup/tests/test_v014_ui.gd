...
func run():
 ...
 await await_reveal(2);expect(view.enemy_nodes[hand.uid].hidden_card,"hand returns to private after one second")
 ...
 expect(view.reveal_player.completed.all(func(row):return row.seconds>=0.98 and row.seconds<1.25),"each face remains public one full second")
