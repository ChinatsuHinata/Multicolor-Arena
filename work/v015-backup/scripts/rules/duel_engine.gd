...
func start(a: Dictionary,b: Dictionary, first_player: int, seed_value: int=0):
 forced_cast={};extra_turns=[];presentation_events.clear();reveal_serial=0
...
func commit_cast(who: int,uid: int,target: Dictionary,plan: Array) -> String:
 ...
 shift(c,"stack"); c.owner=who;c.merge(catalogue_paid,true)
  ...
  "main":
   ...
   for c in units(active):
    ...
    if c.attacked and (DB.has_ability(cards[c.card_id],"brave") or Extra.keyword(self,c,"英勇")) and not Pack.response_locked(self):
     Cat.enqueue(self,{"source":c.duplicate(true),"owner":active,"effect":"untap","optional":false,"name":cards[c.card_id].name+" · 英勇"})
   pump_choices()
  "end": cleanup_end()
 if phase!=old_phase:
...
func possession(palette_uid: int=-1,hand_uid: int=-1):
 ...
 if palette_uid>=0 and hand_uid>=0:
  ...
  players[who].hand.append(p); players[who].palette.append(h);Cat.State.on_palette(self,h)
  note("凭依完成")
