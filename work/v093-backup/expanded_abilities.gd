...
static func keyword(e,c: Dictionary,k: String) -> bool:
 ...
 return k in e.cards[c.card_id].get("keywords",[]) or c.get("modifiers",[]).any(func(m): return m.get(k,false))
...
static func spell_options(e,id: String,who: int) -> Variant:
 ...
 match id:
  ...
  "131":
   ...
   for entry in e.stack:
    if entry.kind=="card" and e.cards[entry.card.card_id].get("spell_type","")=="非符": result.append({"stack_id":entry.id,"mode":"反制非符"})
   return result
...
static func event(e,c: Dictionary,effect: String,optional: bool=false,data: Dictionary={}):
 e.triggers.append({"extended":true,"owner":c.owner,"source":c.duplicate(true),"effect":effect,"optional":optional,"data":data.duplicate(true),"name":e.cards[c.card_id].name})
...
static func on_death(e,c: Dictionary,before: Dictionary):
 ...
 for unit in observers:
  if has(e.cards[unit.card_id],"leader_death_damage") and e.has_leader_ability(unit): event(e,unit,"leader_death_damage",true)
...
static func trigger_options(e,t: Dictionary) -> Array:
 ...
 match t.effect:
  ...
  "returned_spirit_damage","death_damage","leader_death_damage","counter_six":
   if t.effect=="leader_death_damage" and e.usage_count(source.uid,"death_ping")>=2: return []
...
static func resolve_trigger(e,t: Dictionary):
 ...
 match effect:
  ...
  "hand_autumn":
   if not c.is_empty() and c.zone=="hand" and c.epoch==data.ref.epoch and e.field_error(c,who).is_empty(): e.detach(c); e.enter_field(c,who)
  "death_draw_life": e.draw(who); e.players[who].life+=1
  "death_drain": e.players[1-who].life-=2; e.players[who].life+=1
  ...
  "death_undying":
   if dead_valid and e.field_error(dead,who).is_empty():
    e.detach(dead)
    if e.enter_field(dead,who): dead.plus_counters=1
  "death_devour":
   if dead_valid:
    e.move_to(dead,"exile")
    if original: c.plus_counters=c.get("plus_counters",0)+1
  "death_six":
  ...
  "crystal":
   if dead_valid and valid(e,target) and e.find_card(target.uid).tapped:
    var palette=e.find_card(target.uid); e.move_to(palette,"grave"); e.move_to(dead,"palette"); dead.tapped=true
  "leave_ufo": create_token(e,who,"token_ufo")
  "enter_halfghost": create_token(e,who,"token_halfghost")
  "leader_death_damage": e.damage_target(target,1)
  "counter_six": e.damage_target(target,6)
  "sacrifice_choice":
   if valid(e,target): e.move_to(e.find_card(target.uid),"grave")
  "delayed_return":
  ...
  "176":
   for part in t.parts:
    ...
    match part.mode:
     ...
     "横置": e.find_card(part.uid).tapped=true
     "抓一张牌": e.draw(who)
 ...
 match entry.effect:
  ...
  "exile_grave":
   if valid(e,t):
    ...
    if unit: e.players[entry.owner].life+=1; e.players[1-entry.owner].life-=1
  "sacrifice_buff":
