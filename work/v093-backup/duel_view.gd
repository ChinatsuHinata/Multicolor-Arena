...
func highlights() -> Array:
 ...
 if engine.pending.get("owner",-1)==acting_player():
  ...
  if engine.pending.kind=="effect_choice": return engine.pending.options.filter(func(t): return t.has("uid")).map(func(t): return t.uid)
...
func render_prompt():
 ...
 if not local.is_empty():
  ...
  if attack_preview_uid!=0:
   btn("攻击",Rect2(30,703,237,55),confirm_attack,true)
  elif free_main():
...
func choose_target(target: Dictionary):
 if observing or table.combat_animating: return
 if engine.pending.get("kind","")=="effect_choice" and engine.pending.owner==acting_player():
  var options=engine.pending.options.filter(func(t): return t==target or (target.has("uid") and t.get("uid",-1)==target.uid and t.get("epoch",-1)==target.get("epoch",-2)))
  if options.size()==1: engine.choose_effect(options[0]); selection=[]; render()
  elif options.size()>1: choose_options("选择",options,func(chosen): engine.choose_effect(chosen); render())
  return
