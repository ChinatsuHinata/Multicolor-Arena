...
func start(a: Dictionary,b: Dictionary, first_player: int, seed_value: int=0):
 delayed.clear(); combat_queue.clear(); timer_changes.clear(); turn_usage.clear(); cleanup_done=false
...
func start_turn(who: int):
 ...
 phase="reset"; turn_usage.clear(); cleanup_done=false
...
func debug_move(uid: int,destination: String) -> String:
 ...
 if c.get("token",false): shift(c,"void")
  ...
  shift(c,destination)
  if destination!="leader": players[c.owner][destination].append(c)
  if destination=="field":
