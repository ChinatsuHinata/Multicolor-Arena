...
func run():
 ...
 view.hand_clicked(bot.uid); expect(bot.uid in view.selection,"opponent hand selectable")
 view.table.busy_until=0; view.request_cast(bot.uid)
 ...
 await click("你的牌库"); expect(view.debug_open and JSON.stringify(e.pending)==state,"deck inspector opens during required choice")
 ...
 await click("人机牌库"); expect(view.debug_open,"opposing entire deck inspector opens")
