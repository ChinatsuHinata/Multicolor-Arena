...
func sync_hand_nodes(who: int,nodes: Dictionary,parent: Control,available: Array):
 ...
 for i in range(cards.size()):
  var c=cards[i]
  var tile_size=Vector2(146,204) if who==0 else Vector2(70,98) if debug_mode else Vector2(54,75)
  var stride=minf(154,(HAND.size.x-80)/maxi(1,cards.size())) if who==0 else minf(74 if debug_mode else 52,750.0/maxi(1,cards.size()))
  var target=Vector2(HAND.position.x+15+i*stride,680) if who==0 else Vector2(STAGE.get_center().x-tile_size.x/2-(cards.size()-1)*stride/2+i*stride,64)
...
func browse_zone(who: int,zone: String,scroll_position: int=0):
 ...
 txt(("你" if who==0 else "人机")+" · "+ZONE_NAMES[zone]+" · %d" % cards.size(),Rect2(10,10,158,35),18,host.GOLD,browser_panel)
