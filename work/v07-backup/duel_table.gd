...
func build(engine, provider: Callable, mat_path: String, payment_reservations: Array=[], selection: Array=[]):
 ...
 board=plane(self,Vector3.ZERO,Vector2(23,16),mat); board.name="Playmat3"
...
func zone_position(zone: String, who: int) -> Vector3:
 ...
 match zone:
  "deck": return Vector3(9.65*side,0.8,4.0*side)
  "grave": return Vector3(9.65*side,0.25,0.8*side)
  "leader","return_pending": return Vector3(-9.4*side,0.1,3.1*side)
  "hand": return Vector3(0,1,10*side)
  "palette": return Vector3(0,0.1,6*side)
 return Vector3.ZERO
...
func card_snapshot() -> Dictionary:
 ...
 for p in duel.players:
  ...
  for zone in ["deck","hand","field","palette","grave"]:
...
func description(c: Dictionary, at: Vector3, scale_value: float=1.0) -> Dictionary:
 ...
 return {"key":"card_"+str(c.uid),"card_id":c.card_id,"uid":c.uid,"owner":c.owner,"zone":c.get("zone","palette"),"at":at,"rotation":Vector3(0,PI/2 if tapping else 0,0),"scale":Vector3.ONE*scale_value,"kind":"object","gold":c.uid in highlighted or c.uid in chosen or reserved.any(func(r): return r.uid==c.uid)}
...
func layout() -> Dictionary:
 ...
 for who in range(2):
  ...
  if p.leader.zone in ["leader","return_pending"]:
   var d=description(p.leader,zone_position("leader",who)); result[d.key]=d
  var unit_index=0
  var item_groups={}
  for c in p.field:
   var d
   if duel.is_unit(c):
    var at=Vector3(-5.4+(unit_index%3)*2.6,0.1,(2.3+(unit_index/3)*2.2)*side)
    d=description(c,at); unit_index+=1
   else:
    var group=["164","165","167","170"].find(c.card_id)
    if group<0: group=3
    var n=item_groups.get(group,0)
    d=description(c,Vector3(0.3+group*1.9+n*0.22,0.12+n*0.045,3.6*side),0.64)
    item_groups[group]=n+1
   if c.uid in chosen: d.at.y+=0.24
   result[d.key]=d
  for i in range(p.palette.size()):
   var d=description(p.palette[i],Vector3(-6.1+(i%8)*1.67,0.1+(i/8)*0.1,(6.1+(i/8)*0.25)*side),0.7)
   result[d.key]=d
  if p.potato:
   var c={"uid":-100-who,"card_id":"potato","owner":who,"zone":"palette","tapped":false}
   var d=description(c,Vector3(7.4,0.1,6.1*side),0.7); result[d.key]=d
...
func sync(payment_reservations: Array=[], selection: Array=[], available: Array=[], animate: bool=true):
 ...
 for key in next:
  ...
  node.get_node("Outline").visible=d.gold
...
func create_visual(d: Dictionary) -> Node3D:
 ...
 plane(root,Vector3(0,-0.02,0),CARD_SIZE+Vector2(0.18,0.18),material("#edc363")).name="Outline"
 plane(root,Vector3.ZERO,CARD_SIZE,material(d.card_id)).name="Face"
...
func move_card(key: String,node: Node3D,d: Dictionary,animate: bool):
 ...
 tweens[key]=tween; animation_count+=1; mark_busy()
func mark_busy(): busy_until=Time.get_ticks_msec()+int(animation_duration*1000)
func is_animating() -> bool: return Time.get_ticks_msec()<busy_until
...
func update_pile(key: String,cards: Array,at: Vector3,face_up: bool):
 ...
 if not piles.has(key):
  ...
  var mesh=BoxMesh.new(); mesh.size=Vector3(CARD_SIZE.x,LAYER_HEIGHT*0.88,CARD_SIZE.y)
  mm.mesh=mesh; layers.multimesh=mm; layers.material_override=material("#706f79")
  root.add_child(layers)
  plane(root,Vector3.ZERO,CARD_SIZE,material("back")).name="Top"
  var hit=area(root,Vector3.ZERO,Vector3(CARD_SIZE.x,0.2,CARD_SIZE.y)); hit.name="Hit"
 ...
 hit.set_meta("caption",("墓地" if face_up else "牌库")+" · %d 张" % count)
