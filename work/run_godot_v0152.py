...
for name in sys.argv[1:]:
 ...
 label=Path(path).stem;log=root/'work/v0152'/f'{label}.log'
 if visual:log=root/'work/v0152'/f'{label}-visual.log'
 args=[exe]+([] if visual else ['--headless'])+['--path',str(root),'--script',path,'--log-file',str(log)]
 try:
  p=subprocess.run(args,env=env,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,timeout=60,creationflags=subprocess.CREATE_NO_WINDOW)
...
(root/'work/v0152/last-results.json').write_text(json.dumps(results,ensure_ascii=False,indent=2),encoding='utf-8')
