...
func sync():
 ...
 if next!=signature:
  ...
  for entry in entries:
   ...
   var tile=Panel.new();tile.custom_minimum_size=Vector2(218,156) if view.host.landscape_card(card.card_id) else CARD_SIZE;tile.mouse_default_cursor_shape=Control.CURSOR_POINTING_HAND
   tile.set_meta("uid",card.uid);tile.set_meta("card_id",card.card_id);row.add_child(tile)
   var art=TextureRect.new();art.texture=view.host.preview_texture(card.card_id);art.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
...
func animate_ability(id:int,source:Dictionary):
 ...
 var art=TextureRect.new();art.texture=view.host.preview_texture(source.card_id);art.expand_mode=TextureRect.EXPAND_IGNORE_SIZE
 ...
 var face=tiles[id].art;face.modulate.a=0;view.table.mark_busy()
 ...
 tween.chain().tween_callback(func():
  art.queue_free()
  if is_instance_valid(face):face.modulate.a=1)
