...
func battle_ui():
 ...
 container.position=Vector2(0,65)
 container.size=Vector2(1600,785)
 container.stretch=true
 screen.add_child(container)
 var viewport=SubViewport.new()
 viewport.size=Vector2i(1600,785)
 viewport.own_world_3d=true
 viewport.msaa_3d=Viewport.MSAA_4X
 viewport.physics_object_picking=true
 viewport.render_target_update_mode=SubViewport.UPDATE_ALWAYS
 container.add_child(viewport)
 table_3d=preload("res://scripts/battle_table.gd").new()
 viewport.add_child(table_3d)
 table_3d.build(battle,decks[player_choice],decks[ai_choice],texture,battlefield_background)
 table_3d.field_selected.connect(func(index,enemy):
  if enemy: return
  attack(index))
 table_3d.pile_selected.connect(inspect_pile)
 label(screen,"极 彩",Rect2(28,14,200,42),25,GOLD)
 label(screen,"回合 %d   能量 %d / 3" % [battle.round,battle.energy],Rect2(620,20,500,40),23)
 button(screen,"投降",Rect2(1440,18,130,44),func(): confirm_action("确定投降？",func(): battle.clear(); setup()))
 var hand_scroll=ScrollContainer.new()
 hand_scroll.position=Vector2(330,732)
 hand_scroll.size=Vector2(1000,167)
 ...
 for i in range(battle.phand.size()):
  ...
  tile.custom_minimum_size=Vector2(102,150)
  hand_row.add_child(tile)
  card(tile,battle.phand[i],Rect2(0,0,102,143),func(): play_card(i))
 button(screen,"结束回合",Rect2(1380,810,195,60),end_turn,true)
...
func visual_review():
 ...
 battle.pdeck.clear()
 battle.adeck.clear()
 for i in range(50): battle.pdeck.append("70")
 for i in range(30): battle.adeck.append("68")
 battle.pgrave.clear()
 for i in range(20): battle.pgrave.append("165")
 battle.agrave=["99","99","99","99","99"]
 battle.pfield=["70","68"]
 battle.afield=["68","70"]
 battle_ui()
 await capture_review("battle-stacks")
 get_tree().quit()
...
func inspect_pile(which: String):
 if which in ["pgrave","agrave"]: table_3d.inspect_cards(battle[which])
 elif which=="pside": table_3d.inspect_cards(decks[player_choice].side)
