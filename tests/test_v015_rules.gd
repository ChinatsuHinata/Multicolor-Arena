...
func audit_entries():
 ...
 for id in ids:
  ...
  # Enters-with counters is a replacement, not a triggered ability.
  if id=="new-spx-001":continue
...
func audit_activations():
 ...
 for id in ids:
  ...
  if id=="new-spx-005":put("new-spx-007")
  if id=="new-spx-003":e.Cat.token(e,0,"青蛙",4,4,2,["蓝","绿"],["不占战场格"])
  ...
  for action in actions:
   ...
   if key=="n21:frog_counter":stacked("96",1,e.ref_target(c))
