...
func capture(name: String):
 ...
 root.get_texture().get_image().save_png("res://work/v0181/"+name+".png")
func run():
 ...
 authority_session.initialize("res://work/v0181/ui-host");client_session.initialize("res://work/v0181/ui-guest")
 ...
 await until(func():return client_session.metrics.local_ms>=0 and client_session.metrics.remote_ms>=0)
 ...
 expect(is_instance_valid(view.network_latency_label) and "ms" in view.network_latency_label.text,"battle shows measured latency for both players")
 expect(view.local_seat==1 and view.acting_player()==1,"guest acts as stable seat one")
 expect(view.hand_nodes.size()==4 and view.enemy_nodes.is_empty(),"guest sees own hand without opposing hand tiles")
 expect(view.hand_nodes.values().all(func(n):return not n.hidden_card) and view.hud.get_children().any(func(n):return n is Label and n.text=="手牌 4"),"guest sees own faces and opposing hand count")
 ...
 print("V0181 UI ",checks," checks; ",failures," failures")
