...
func run():
 ...
 await click(view.hand_nodes[spell.uid].get_global_rect().get_center())
 view.choose_target({"player":1}); view.choose_target(e.ref_target(enemy)); await finish_group(); await press("确定")
 ...
 await press("确定"); expect(spell.zone=="stack" and spell.cast_x==2,"X payment is committed with the card")
 ...
 await click(view.hand_nodes[spell.uid].get_global_rect().get_center()); await press("确定")
 ...
 view.execute_action(actions.filter(func(a): return a.type=="direct_attack")[0]); view.choose_target(e.ref_target(enemy)); await press("确定")
