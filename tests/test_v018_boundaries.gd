...
func run():
 ...
 check(series.ready(0).is_empty() and series.state.coin.is_empty(),"ready waits for opponent before coin toss")
 series.ready(1);check(series.start_game() and series.state.coin.first==series.state.first,"both ready starts game with authoritative coin")
