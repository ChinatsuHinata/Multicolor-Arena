...
func run():
 var e=Session.Duel.new();var decks=JSON.parse_string(FileAccess.get_file_as_string(Store.SAVE_PATH)).decks;e.start(decks[0],decks[1],0,729)
 for p in e.players:
 ...
 check(series.ready(0).is_empty() and series.state.roll.is_empty(),"ready waits for opponent before dice roll")
 series.ready(1);check(series.prepare_choice() and series.state.roll["values"][0]!=series.state.roll["values"][1],"both ready rolls dice and breaks ties")
 check(not series.start_game() and not series.choose_first(1-series.state.chooser,true).is_empty(),"game waits for authorized choice")
 check(series.choose_first(series.state.chooser,true).is_empty() and series.start_game() and series.state.first==series.state.chooser,"chooser starts first game")
