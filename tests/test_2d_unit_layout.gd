...
func run():
 ...
 for i in range(8):put("164","field")
 view.render()
 await settle()
 var crowded_items=view.table.descriptors.values().filter(func(d):return d.zone=="field" and d.group=="item" and d.owner==0)
 crowded_items.sort_custom(func(a,b):return a.at.x<b.at.x)
 expect(crowded_items.size()==12 and crowded_items.all(func(d):return fits_zone("item",d.at,0,Vector2.ONE*view.table.CARD_SIZE.y*d.scale.x*0.5)),"twelve items fit inside their zone even when tapped")
 expect(crowded_items.all(func(d):return d.get("fanned",false) and d.get("copy_total",0)==12) and crowded_items[0].at.x<crowded_items[-1].at.x,"crowded items fan out with distinguishable copy numbers")
 ...
 expect(view.table.WIDE_ZONES.item.has_point(Vector2(view.table.item_placement(0,4,0).at.x,view.table.item_placement(0,4,0).at.z)) and view.table.WIDE_ZONES.support.has_point(Vector2(view.table.support_placement(0,2,0).at.x,view.table.support_placement(0,2,0).at.z)),"3D item and barrier rows follow the divided zones")
