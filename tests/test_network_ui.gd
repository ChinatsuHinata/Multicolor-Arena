...
func capture(name: String):
 ...
 root.get_texture().get_image().save_png("res://work/network-current/"+name+".png")
...
func run():
 ...
 authority_session.initialize("res://work/network-current/ui-host");client_session.initialize("res://work/network-current/ui-guest")
 app.lan_session=client_session;app.online();await process_frame
 authority_session.create_room(3,true,47982,"127.0.0.1","test");client_session.join_room("127.0.0.1",47982)
 ...
 for seat in [0,1]:
  ...
  expect(Store.validate(decks[seat],true,"test").is_empty(),"strict legal sideboard fixture")
 ...
 await until(func():return not client_session.latest_snapshot.is_empty() and authority_session.can_act(true) and client_session.can_act(true));sync_sessions()
 ...
 var instructions=app.screen.get_node_or_null("SideboardInstructions") as Label
 expect(instructions!=null and instructions.text.contains("拖到另一卡组") and instructions.text.contains("移动 1 张"),"sideboarding explains that cards move between decks")
 ...
 var reserve_id=reserve.card_id
 await drag(reserve.get_global_rect().get_center(),app.main_content.get_global_rect().position+Vector2(820,465))
 expect(app.draft.main==initial.main.slice(1)+[reserve_id] and app.draft.side==initial.side.slice(1)+[first_id],"drag removes one exact side card and adds it to main")
 var last_main=deck_tile(app.deck_canvas,"main",app.draft.main.size()-1)
 await click(last_main.get_global_rect().get_center())
 expect(app.draft.main.size()==49 and app.draft.side.size()==11 and app.draft.main.count(reserve_id)==initial.main.slice(1).count(reserve_id),"click also moves a card without creating a copy")
 var last_side=deck_tile(app.deck_canvas,"side",app.draft.side.size()-1)
 var side_scroll=last_side.get_parent()
 while side_scroll!=null and not side_scroll is ScrollContainer:side_scroll=side_scroll.get_parent()
 if side_scroll is ScrollContainer:side_scroll.ensure_control_visible(last_side)
 await process_frame
 await click(last_side.get_global_rect().get_center())
 expect(app.draft.main.size()==50 and app.draft.side.size()==10,"clicking the side card moves it back to main")
 expect(app.sideboard_status.text.is_empty(),"editing clears obsolete validation message")
 await tap("排序卡组")
 expect(Session.Series.sideboard_error(app.draft,original,true,client_session.room.rule_set).is_empty(),"completed exchange preserves registered multiset and strict rules")
 ...
 expect(view.network_status_label.text.contains("秒") and find_button(view.ui,"不再等待，离开对局")!=null and not view.modal,"countdown leaves battlefield visible and offers an exit")
 ...
 expect(not client_session.ended() and view.network_status_label.text.contains("是否继续等待"),"30 seconds shows the wait decision on the battlefield")
 expect(find_button(view.ui,"继续等待")!=null and find_button(view.ui,"不再等待，离开对局")!=null and find_button(view.ui,"保留")==null,"waiting offers both choices while play stays locked")
 await capture("disconnect-ended")
 await tap("继续等待")
 expect(client_session.wait_choice_confirmed and not client_session.wait_choice_pending and find_button(view.ui,"不再等待，离开对局")!=null,"continuing wait has no deadline and keeps an exit option")
 await tap("不再等待，离开对局")
 expect(app.page=="online" and client_session.room_id.is_empty() and client_session.identity.resume.is_empty(),"leaving waiting returns to the room list without a result")
 ...
 print("NETWORK UI ",checks," checks; ",failures," failures")
