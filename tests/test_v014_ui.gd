...
func run():
 ...
 view.render();await settle();expect(view.enemy_nodes.is_empty() and view.hud.get_children().any(func(n):return n is Label and n.text=="手牌 3"),"opposing hand shows only its count")
 ...
 expect(view.enemy_nodes.is_empty() and view.reveal_player.public_uids==[hand.uid] and view.reveal_player.face.texture==app.texture(hand.card_id),"only selected hand card temporarily public")
 expect(view.reveal_player.zone_rect("hand",1,hand.uid,true).get_center()==view.OPPONENT_HAND_COUNT.get_center(),"hidden hand animations use the count position")
 await capture("hand-reveal");await click(Vector2(1480,800));expect(e.revision==revision,"revealing pauses further actions")
 await await_reveal(2);expect(view.enemy_nodes.is_empty(),"hand returns to count-only display after the presentation")
