...
func run():
 ...
 expect(app.draft.main==[source_id],"mouse drag from library adds exactly one card")
 if main_tile()!=null:
 ...
 expect(app.draft.side==[source_id],"mouse drag from library adds to side deck")
 app.return_card_to_library(Vector2.ZERO,{"card_id":source_id,"source_zone":"side","source_index":0})
 ...
 var ability_text=app.card_description("68")
 expect(not ability_text.is_empty() and "SOI-" not in ability_text and "画师" not in ability_text and "攻击 3" not in ability_text,"preview shows ability text without card metadata")
