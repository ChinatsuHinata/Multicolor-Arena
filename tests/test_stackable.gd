...
func run():
 ...
 clean(true)
 var existing=put("token-fdf-129","field")
 var generated=e.Cat.printed_token(e,0,"token-fdf-129")
 expect(e.stackable_signature(existing)==e.stackable_signature(generated),"默认字段缺省与显式零值的要石状态等价")
 view.render();await settle()
 expect(view.table.descriptors.values().filter(func(d):return d.card_id=="token-fdf-129").size()==1,"新生成的要石并入已有堆叠")
 clean(true)
 stones=[]
 for i in range(3):stones.append(e.Cat.printed_token(e,0,"token-fdf-129"))
 var kosuzu=e.make_card("character-rec-104",1,"hand")
 e.players[1].hand.append(kosuzu);e.enter_field(kosuzu,1);e.pump_choices()
 var target_options=e.pending.get("options",[]).filter(func(r):return r.get("uid",-1)==stones[1].uid)
 expect(e.pending.get("kind","")=="effect_choice" and target_options.size()==1,"小铃可指定堆叠中的一枚要石")
 if not target_options.is_empty():e.choose_effect(target_options[0])
 view.render();await settle()
 var targeted_layout=view.table.descriptors.values().filter(func(d):return d.card_id=="token-fdf-129")
 expect(targeted_layout.size()==2 and targeted_layout.any(func(d):return d.uid==stones[1].uid and d.members.size()==1) and targeted_layout.any(func(d):return d.members.size()==2),"堆叠上的目标要石独立显示并可点击")
 var target_key="card_"+str(stones[1].uid)
 var target_rect=view.projected_card_rect(view.table.visuals[target_key])
 var target_arrows=view.stack_target_arrows().filter(func(a):return a.target.get("uid",-1)==stones[1].uid)
 expect(target_arrows.size()==1 and (target_rect.grow(6).has_point(target_arrows[0].to) if not target_arrows.is_empty() else false),"小铃的箭头终点落在独立可互动的要石上")
 view.inspect_card("token-fdf-129",stones[1].uid)
 await process_frame
 await capture("kosuzu-targeted-stacked-stone")
 var newer=e.Cat.printed_token(e,0,"token-fdf-129")
 view.render();await settle()
 targeted_layout=view.table.descriptors.values().filter(func(d):return d.card_id=="token-fdf-129")
 expect(targeted_layout.size()==2 and targeted_layout.any(func(d):return d.members.size()==3 and newer.uid in d.members),"指定期间新生成的要石并入未指定的一组")
 expect(e.priority==0 and e.available_actions(0,stones[1].uid).any(func(a):return a.get("key","")=="token-fdf-129"),"被指定的要石仍可响应并发动能力")
 view.object_clicked(stones[1].uid)
 expect(view.modal and view.modal_root.get_child(0).get_meta("action_card_uid",-1)==stones[1].uid,"点击独立要石打开其行动菜单")
 view.close_overlay()
 expect(e.commit_extension(0,stones[1].uid,{"player":0},[],"token-fdf-129").is_empty(),"被指定的要石可牺牲自身发动能力")
 view.render();await settle()
 expect(view.table.descriptors.values().filter(func(d):return d.card_id=="token-fdf-129").size()==1,"目标要石离场后剩余要石重新合并")
 clean(true)
 var items=[]
 for i in range(3):items.append(e.Cat.printed_token(e,0,"token-fdf-127"))
 var star=e.make_card("3",1,"hand")
 e.players[1].hand.append(star);e.enter_field(star,1);e.pump_choices()
 target_options=e.pending.get("options",[]).filter(func(r):return r.get("uid",-1)==items[1].uid)
 expect(e.pending.get("kind","")=="effect_choice" and target_options.size()==1,"斯塔可指定堆叠中的一个道具")
 if not target_options.is_empty():e.choose_effect(target_options[0])
 view.render();await settle()
 var item_layout=view.table.descriptors.values().filter(func(d):return d.card_id=="token-fdf-127")
 expect(item_layout.size()==2 and item_layout.any(func(d):return d.uid==items[1].uid and d.members.size()==1),"斯塔指定的道具独立显示")
