...
func run():
 ...
 await click(view.hand_nodes[unit.uid].get_global_rect().get_center()); await press("发动")
 ...
 e.move_to(shrine,"grave"); view.render(); view.update_inspection()
 expect(not view.inspection_text.get_meta("leader_enabled"),"removing shrine greys the ability again")
 leader.leader_counters=1; view.render(); view.update_inspection()
 ...
 expect(view.stack_panel.get_parent()==view.ui and view.stack_panel.AREA.position.x>=view.SIDEBAR.position.x-8,"stack rail is a separate screen-space control")
 ...
 await press("发动"); resolve(); expect(mokou.zone=="hand","new Mokou grave ability returns itself to hand")
 ...
 print("V095_TEST: ",checks," checks; ",failures," failures"); quit(0 if failures.is_empty() else 1)
