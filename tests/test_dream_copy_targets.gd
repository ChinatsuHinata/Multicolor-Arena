...
func run():
 ...
 fresh();own=add("53",0);foe=add("53",1);var foe2=add("53",1)
 ...
 check(e.pending.options.any(func(o):return o.get("uid",-1)==foe2.uid and o.get("retarget_paid_base",{}).get("sacrifice",{})==original_target.sacrifice),"已支付牺牲的符卡复制品可以另选目标而不重付费用")
 var picker=preload("res://scripts/target_picker.gd").new();picker.configure(e.pending.options,"paid_sacrifice")
 check(picker.available_refs().all(func(r):return r.get("uid",-1)!=own.uid),"复制品的选目标界面不再要求重选已牺牲单位")
 new_target=e.pending.options.filter(func(o):return o.get("uid",-1)==foe2.uid)[0]
 choose_copy(new_target);choose_copy(new_target)
 resolve_top();resolve_top();resolve_top()
 check(own.zone=="grave" and foe.damage==5 and foe2.damage==10,"秋意ノ溪只牺牲一次且三次伤害各结算到指定目标")
 fresh();own=add("53",0);var grave1=add("53",0,"grave");var grave2=add("53",0,"grave")
 ...
 new_target=e.pending.options.filter(func(o):return o.get("uid",-1)==grave2.uid)[0]
 check(new_target.get("retarget_paid_base",{}).get("sacrifice",{})==original_target.sacrifice,"萤燈ノ森复制品可以在已支付牺牲后重选墓地目标")
 choose_copy(new_target)
 new_target=e.pending.options.filter(func(o):return o.get("uid",-1)==own.uid)[0]
 choose_copy(new_target)
 resolve_top();resolve_top();resolve_top()
 check(own.zone=="field" and grave1.zone=="field" and grave2.zone=="field","萤燈ノ森三次结算各移回选定单位")
