...
func run():
 ...
 host.initialize("res://work/v018/session-host-"+run_id);guest.initialize("res://work/v018/session-guest-"+run_id)
 ...
 check(await until(func():return host.series.state.status=="choosing" and guest.room.get("status","")=="choosing" and host.can_act() and guest.can_act()),"first-player choice synchronized")
 (host if host.series.state.chooser==0 else guest).room_action({"name":"first","first":true})
 ...
 check(host.series.state.status=="between" and host.series.state.roll_history.size()==1,"single-game result enters between-game preparation")
 ...
 check(await until(func():return host.series.state.status=="choosing" and guest.room.get("status","")=="choosing" and host.can_act() and guest.can_act()),"loser receives game-two choice")
 (host if host.series.state.chooser==0 else guest).room_action({"name":"first","first":true})
